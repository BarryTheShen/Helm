/**
 * discover.ts — Auto-discover backend routes, frontend pages, mobile components,
 * templates, and action registry entries.
 *
 * Called from globalSetup.ts after authentication. Writes qa/src/discovered.json.
 */
...
async function discoverEndpoints(token: string): Promise<any[]> {
  const data = await fetchJSON(`${BACKEND_BASE}/openapi.json`, token);
  if (data._error || !data.paths) return [];
  return Object.entries(data.paths).flatMap(([path_, methods]: [string, any]) =>
    Object.keys(methods)
...
async function discoverComponents(token: string): Promise<any> {
  const data = await fetchJSON(`${BACKEND_BASE}/api/components/registry`, token);
  return data;
}

async function discoverTemplates(token: string): Promise<any> {
  const data = await fetchJSON(`${BACKEND_BASE}/api/templates`, token);
  return data;
}
...
function discoverActions(): string[] {
...
function discoverMobileComponents(): Record<string, string[]> {
