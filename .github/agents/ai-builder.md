...
- `user-invocable` - Set to `false` for sub-agents that shouldn't appear in agent picker (default: `true`)
...
2. Create **sub-agents** for each phase or specialty, using `.subagent.agent.md` naming and `user-invocable: false`.
...
**Orchestrator Agent (user-invocable):**
```markdown
...
Sub-agents must set `user-invocable: false` and must **not** include `handoffs` — handoff buttons are only rendered for user-facing agents. Sub-agents cannot communicate with the user directly; they return output (including any questions) to the orchestrator.
...
- [ ] Sub-agents use `.subagent.agent.md` naming and `user-invocable: false`
...
- Agents MUST be in `<provider>/agents/` (no subdirectories)
  ...
  - Sub-agents: use `.subagent.agent.md` extension and set `user-invocable: false`
...
- For multi-agent workflows:
  - ALWAYS create a descriptively named orchestrator agent (user-invocable)
  - All phase agents must be sub-agents with `user-invocable: false` and no `handoffs`
