...
| WebSocket handling | `routers/websocket.py` + `services/websocket_manager.py` | `websocket.py`: token in query param; device_id tracked; module_action dispatches to action registry. `websocket_manager.py`: device-aware singleton |
...
SDUI actions are dispatched via `useActionDispatcher` hook — fully wired as of Session 2 (see Action Execution Flow below).

### Draft/Approval Flow (Human-in-the-Loop)

When the AI sets a screen, it goes through a draft phase requiring human approval:

```
1. AI calls set_screen(module_id, screen, draft=True) via MCP or agent proxy
2. Backend stores screen JSON at key `sdui__<module_id>__draft` in module_states
3. Sends {type: "sdui_draft_update", module_id: "home", screen: {...}} over WebSocket
4. Frontend useSDUIScreen hook receives event and sets draft state
5. home.tsx shows DraftPreview component with preview, approve/reject buttons, and feedback input
6. User clicks Approve: POST /api/sdui/{module_id}/draft/approve
   - Draft deleted from DB, promoted to live screen at sdui__<module_id>
   - sdui_screen_update WS event fired — live screen appears immediately
7. User clicks Reject: POST /api/sdui/{module_id}/draft/reject (with optional feedback)
   - Draft deleted, sdui_draft_rejected WS event fired
   - DraftPreview disappears
```

### Action Execution Flow (SDUI server_action)

SDUI components can trigger named server-side functions instead of raw API calls:

```
1. User taps a button with {type: "server_action", function: "submit_form", params: {...}}
2. useActionDispatcher hook in the tab screen receives the action
3. Calls ApiClient.executeAction(functionName, params): POST /api/actions/execute
4. Backend validates function name against registry whitelist (returns 404 if unknown)
5. Registry handler called with (user_id, params, db) — scoped to the authenticated user
6. Handler returns result dict; frontend receives {status: "ok", result: {...}}
```

Other action types dispatched by useActionDispatcher:
- `navigate`: router.push() to mapped tab route
- `go_back`: router.back()
- `open_url`: Linking.openURL() (scheme whitelist: http, https, mailto, tel)
- `copy_text`: expo-clipboard Clipboard.setStringAsync()
- `send_to_agent`: sends WebSocket chat_message + navigates to chat tab
- `toggle`: no-op at dispatcher level (state managed locally by component)
- `legacy api_call`: backward compat, treated as server_action
...
- `sdui__<module_id>__draft` — pending draft screen awaiting human approval
- `_tabs_config` — JSON list of hidden tab IDs
- `device_config` — device layout config (tab_bar_modules, default_module, nav_mode)
- `form_data__<form_id>` — form submission history (stored by `submit_form` action handler)
...
| GET | `/api/sdui` | Yes | `routers/modules.py` — list all active SDUI screens |
| GET | `/api/sdui/{module_id}/draft` | Yes | `routers/modules.py` — get pending draft |
| POST | `/api/sdui/{module_id}/draft/approve` | Yes | `routers/modules.py` — promote draft to live |
| POST | `/api/sdui/{module_id}/draft/reject` | Yes | `routers/modules.py` — discard draft |
| POST | `/api/actions/execute` | Yes | `routers/actions.py` — execute named function |
| GET | `/api/actions/functions` | Yes | `routers/actions.py` — list registered functions |
...
2. ~~**SDUI actions never execute**~~ — **FIXED (Session 2)**: `useActionDispatcher` hook now wires all 7 action types to real handlers (`navigate`, `go_back`, `open_url`, `copy_text`, `server_action`, `send_to_agent`, `toggle`). All 7 tab screens use the hook.
