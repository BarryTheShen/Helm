...
| Need to change... | Edit this file | Notes |
|-------------------|---------------|-------|
| Playwright config | `qa/playwright.config.ts` | 2 projects: `backend-only` (api-*.spec.ts) and `e2e` (everything else) |
| Package / scripts | `qa/package.json` | ESM, scripts: `test`, `test:backend`, `test:e2e` |
| Server auto-start | `qa/src/globalSetup.cjs` | Starts backend+Vite if not running, handles auth, runs discovery. MUST be `.cjs` |
| Server auto-stop | `qa/src/globalTeardown.cjs` | Kills servers if QA started them. MUST be `.cjs` |
| Discovery scanner | `qa/src/discover.cjs` | Scans backend routes, actions, components, templates → `discovered.json` |
...
| Convenience script | `qa/run.sh` | Runs backend pytest + Playwright, outputs to `qa/results/` |

See `docs/codebase-explanation/qa.md` for full details on architecture, gotchas, and test coverage.
...
| Backend API | ✅ QA suite (`qa/`) — `api-auth.spec.ts` + `api-endpoints.spec.ts` in `backend-only` project |
